...
const DASHBOARD_KEY = "flywheel-dash-v3";
const SCANNER_KEY = "flywheel-scanner-v1";
const RESULTS_KEY = "flywheel-results-v1";
    ...
    try {
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: `You are a roll opportunity scanner for the Flywheel Playbook — a covered call income methodology. Your job is to provide actionable roll analysis for a specific short option position, classifying the appropriate Mode (1, 3, 4, or none) before recommending action.

ROLL MODE TAXONOMY:
- Mode 1 (Standard 21-DTE Management): scheduled roll at the 21-DTE trigger or 50%+ profit close. Requires 50% net credit standard.
- Mode 3 (Offensive Roll): triggered by a rally with bull-trap characteristics, executed during Amateur Hour, requires 50% net credit standard.
- Mode 4 (Calendar Correction): triggered by exogenous calendar shift (typically an earnings date confirmation that displaces the original exit catalyst out of contract life). Authorization requires: (a) original strike+DTE was selected with a specific volatility catalyst as the planned exit mechanism, (b) catalyst date has confirmed to move outside current DTE, (c) new DTE places catalyst back inside contract life, (d) net debit is bounded. Mode 4 EXPLICITLY ACCEPTS NET DEBIT — the 50% net credit standard does NOT apply. Discipline rules: single-roll limit per displaced catalyst (no recursive Mode 4), pre-committed stopping rule before execution, bounded debit relative to realistic premium recovery.

TASK 1 — CALENDAR SHIFT CHECK:
Review the earnings calendar context. Did an earnings or other catalyst date confirmation move OUTSIDE the current contract's DTE window? If yes, flag CALENDAR SHIFT DETECTED and route to Mode 4 evaluation. If no, proceed with Mode 1/3 path.
...
- For Mode 1/3: delta ≤ 0.22, strike at or above current strike (up and out)
- For Mode 4: relax delta constraint (strike may be ATM or modestly ITM by design); strike chosen to absorb price action that occurred during the dislocation window; new DTE must place displaced catalyst back inside contract life
List each candidate with: expiration, strike, delta, bid price, and net credit-or-debit vs. the buy-to-close cost.

TASK 4 — MODE-APPROPRIATE RECOMMENDATION:
- Mode 1/3 path: Apply the 50% net credit standard. Does any candidate produce ≥ 50% of original premium ($${pos.premium}/share) as net credit? If yes, QUALIFYING. If best available is below 50%, SUBTHRESHOLD — recommend hold/expire/close instead.
- Mode 4 path: Score on catalyst-restoration fit, bounded debit, and discipline checks. Output MUST include: (a) the explicit gap-through-strike scenario the operator must pre-commit to as the stopping rule, (b) confirmation that this is the FIRST Mode 4 roll on this position (recursive Mode 4 is forbidden — recommend accept assignment if the position has already been calendar-corrected once).

OUTPUT REQUIREMENTS:
1. Mode classification (1/3/4/CLOSE/HOLD/ASSIGN) at the top of the response
2. Calendar shift status (DETECTED / NOT DETECTED / N/A)
3. BTC cost estimate with intrinsic/extrinsic split
4. Ranked candidates with the appropriate metric (net credit for Mode 1/3, catalyst-restoration + bounded debit for Mode 4)
5. Final recommendation with explicit reasoning
6. For Mode 4: the pre-committed stopping rule the operator must accept before executing
...
Be precise with numbers. If exact chain data isn't available, use the best estimates from current IV levels and state your confidence level.`,
          messages: [{ role: "user", content: `Scan roll opportunities for my position:\n${posDesc}${earningsContext}\n\nSearch for current ${pos.underlying} options chain data. Determine which Mode applies (1/3/4 or no roll). For Mode 1/3, find buy-to-close pricing and sell-to-open candidates at delta ≤ 0.22 with DTE between ${pos.dte + 1} and 45 days, applying the 50% net credit standard against my original premium of $${pos.premium}/share. For Mode 4 (calendar correction), find candidates that restore the displaced catalyst inside contract life with bounded net debit, and surface the required pre-committed stopping rule.` }],
        }),
      });
...
Be specific with numbers and dates. State when data is estimated vs. confirmed.`,
  ...
  const scanAll = async () => {
    ...
    const earningsContext = earningsCalendar
      ? `\n\nEARNINGS CALENDAR CONTEXT:\nNext ${earningsCalendar.ticker || "NVDA"} earnings: ${earningsCalendar.next?.date || "unknown"} (${earningsCalendar.next?.confirmed ? "confirmed" : "estimated"})${earningsCalendar.next?.shifted ? `\nSHIFT DETECTED: prior estimate was ${earningsCalendar.next.priorEstimate}, confirmed ${earningsCalendar.next.confirmedAt}. Any open NVDA short call whose original DTE assumed the prior date as planned exit catalyst is a Mode 4 Calendar Correction candidate.` : ""}\nCurrent calendar position: T${earningsPosition?.daysDelta >= 0 ? "-" : "+"}${Math.abs(earningsPosition?.daysDelta || 0)} (${earningsPosition?.window || "unknown window"})`
      : "";
    try {
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          tools: [{ type: "web_search_20250305", name: "web_search" }],
          system: `You are a roll opportunity scanner for the Flywheel Playbook — a covered call income methodology. Analyze ALL provided short option positions and classify each into the appropriate Mode (1/3/4/CLOSE/HOLD/ASSIGN) before recommending action.

ROLL MODE TAXONOMY:
- Mode 1 (Standard 21-DTE Management): scheduled roll at the 21-DTE trigger or 50%+ profit close. Requires 50% net credit standard.
- Mode 3 (Offensive Roll): triggered by a rally with bull-trap characteristics, executed during Amateur Hour. Requires 50% net credit standard.
- Mode 4 (Calendar Correction): triggered by exogenous calendar shift (typically an earnings date confirmation that displaces the original exit catalyst out of contract life). Authorization requires: (a) original strike+DTE was selected with a specific volatility catalyst as planned exit mechanism, (b) catalyst date confirmed to move outside current DTE, (c) new DTE places catalyst back inside contract life, (d) net debit is bounded. Mode 4 EXPLICITLY ACCEPTS NET DEBIT — the 50% net credit standard does NOT apply. Discipline rules: single-roll limit per displaced catalyst, pre-committed stopping rule before execution, bounded debit relative to realistic premium recovery.

For EACH position:
1. **Calendar shift check** — given the earnings calendar context, has a catalyst date moved outside this position's contract life? If yes, route to Mode 4 evaluation. If no, proceed with Mode 1/3 path.
2. **Estimate the buy-to-close cost** with intrinsic/extrinsic split.
3. **Search for roll candidates**: same underlying, CALL options, expiration LATER than current but ≤ 45 DTE.
   - Mode 1/3: delta ≤ 0.22, strike at or above current strike, only candidates producing NET CREDIT.
   - Mode 4: relax delta constraint, candidates that place the displaced catalyst back inside contract life, prioritize bounded net debit and catalyst-restoration fit.
4. **Apply the appropriate metric**:
   - Mode 1/3: 50% net credit standard. Flag QUALIFYING (≥ 50% of original premium as credit) or SUBTHRESHOLD.
   - Mode 4: bounded-debit acceptance, surface the required pre-committed stopping rule.
5. **Mode 4 single-roll limit**: if a position has already been calendar-corrected once, do NOT recommend a second Mode 4 roll. Recommend accept assignment instead.
...
- 22+ DTE: MONITORING — only flag if a calendar shift makes this a Mode 4 candidate

OUTPUT FORMAT:
Sort by urgency (lowest DTE first). For each position present a compact summary table row: Ticker · Strike/Exp · DTE · Urgency · Mode · BTC · Recommendation · Net Credit-or-Debit · Stopping Rule (Mode 4 only).

Be precise with numbers. State confidence level on pricing estimates.`,
          messages: [{ role: "user", content: `Scan roll opportunities for all my positions:\n${posDescs}${earningsContext}\n\nFor each position, classify the appropriate Mode (1/3/4 or no roll), search current options data, and provide the mode-appropriate recommendation. Mode 1/3 candidates must satisfy the 50% net credit standard against original premiums; Mode 4 candidates must restore a displaced catalyst inside contract life with a bounded net debit and explicit stopping rule. Surface any Mode 4 triggers from the earnings calendar context prominently.` }],
        }),
      });
      const data = await resp.json();
      const text = data.content?.filter(b => b.type === "text").map(b => b.text).join("\n") || "No results.";
      ...
      <div style={{ padding: "16px 24px 0", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <Card glow={scanResult ? C.cyan : null}>
          ...
          {scanResult && !scanLoading && (
            <div style={{ padding: "14px", background: C.bg, borderRadius: 6, border: `1px solid ${C.cyanDim}`, whiteSpace: "pre-wrap", fontSize: 13, fontFamily: font.sans, lineHeight: 1.7, color: C.text, overflowY: "auto" }}>
              {scanResult}
            </div>
          )}
          {!scanResult && !scanLoading && (
        ...
        <Card glow={gammaResult ? C.purple : null}>
          ...
          {gammaResult && !gammaLoading && (
            <div style={{ padding: "14px", background: C.bg, borderRadius: 6, border: `1px solid ${C.purpleDim}`, whiteSpace: "pre-wrap", fontSize: 13, fontFamily: font.sans, lineHeight: 1.7, color: C.text, overflowY: "auto" }}>
              {gammaResult}
            </div>
          )}
          {!gammaResult && !gammaLoading && (
