...
const STORAGE_KEY = "flywheel-macro-v2";
    ...
    <Card style={{ padding: "14px 18px" }} accent={position.shifted ? C.amber : null}>
      ...
      {position.shifted && (
        <div style={{ fontSize: 11, fontFamily: font.sans, color: C.amber, marginTop: 8, padding: "8px 10px", background: C.amberDim + "30", borderLeft: `2px solid ${C.amber}`, borderRadius: 4 }}>
          Date shifted from <span style={{ fontFamily: font.mono }}>{position.priorEstimate}</span> (confirmed {position.confirmedAt}). <strong>Mode 4A</strong>: evaluate writing short-DTE CCs to expire before <span style={{ fontFamily: font.mono }}>{position.date}</span>, then return to standard 45-DTE cadence post-event. Run /scan-rolls to assess open positions.
        </div>
      )}
      {lastPrint && (
...
export default function MacroForceDashboard() {
  ...
  const earningsPosition = useMemo(() => {
    ...
    const today = new Date(); // always use today's live date for countdown
            ...
            <Card accent={cat} glow>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                    ...
                    <Badge color={stateColor}>{force.state}</Badge>
                    <Badge color={force.direction_bias === "bullish" ? C.green : force.direction_bias === "bearish" ? C.red : C.gray}>
                      {force.direction_bias === "bullish" ? "▲" : force.direction_bias === "bearish" ? "▼" : "●"} {force.direction_bias.toUpperCase()}
                    </Badge>
                  </div>
                  <div style={{ display: "flex", gap: 16, fontSize: 11, fontFamily: font.mono, color: C.textDim, flexWrap: "wrap" }}>
                    ...
                    <span>net YTD <strong style={{ color: force.net_ytd_reaction > 0 ? C.green : force.net_ytd_reaction < 0 ? C.red : C.gray }}>{force.net_ytd_reaction >= 0 ? "+" : ""}{force.net_ytd_reaction.toFixed(2)}</strong></span>
                    <span>n={force.events_total} events ({force.events_bullish}▲ {force.events_bearish}▼)</span>
                    <span>trend: {force.attenuation_trend}</span>
                    {force.last_event_date && <span>last event: {force.last_event_date}</span>}
                  </div>
                </div>
              ...
              {forceEvents.length > 0 ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 340, overflowY: "auto" }}>
                  <div style={{ fontSize: 10, fontFamily: font.mono, color: C.textMuted, letterSpacing: "0.1em", marginBottom: 2 }}>
                    {forceEvents.length} ATTRIBUTED EVENT{forceEvents.length !== 1 ? "S" : ""}
                  </div>
                  {forceEvents.map(ev => (
                ...
                <div style={{ padding: "16px 0", color: C.textMuted, fontFamily: font.mono, fontSize: 12 }}>
                  No events attributed to this force in the current dataset.
                </div>
              )}
            </Card>
          </div>
